...
func LocalStartRecorder(recorderPath, tcpPort, resolution, bitrate, frameRate, codec, backend string) error {
	// 确保文件有执行权限（建议在部署阶段做好，而不是在代码里每次都 chmod）
	os.Chmod(recorderPath, 0755)

	// 直接执行二进制，不要通过 bash -c 拼接字符串
	execCmd := exec.Command(recorderPath,
